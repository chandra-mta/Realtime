...
set file2 = ftp://mussel.srl.caltech.edu/pub/ace/browse/EPAM/EPAM_quicklook.txt

/bin/rm $SPACE_Wdir/returned_fast
wget $file  -o/dev/null -O$SPACE_Wdir/returned_fast
wget $file2 -o/dev/null -O$WEBdir/returned_fast_3d
...
if ($count > 28) then 
   ...
   /bin/rm $SPACE_Wdir/lasthour_fast
    tail -72 $SPACE_Wdir/returned_fast > $SPACE_Wdir/lasthour_fast
    head -72 $SPACE_Wdir/lasthour_fast > $SPACE_Wdir/last45_fast
    ...
    tail -n+13 returned_fast >> fast.archive
    sort -u fast.archive -o fast.archive
    #smart_proc#
...
wget -q -O$WEBdir/wingkp.gif http://services.swpc.noaa.gov/images/wing-kp-24-hour.gif 
