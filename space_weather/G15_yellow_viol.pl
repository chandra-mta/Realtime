...
$infile="$dir_vars{'SPACE_Wdir'}/G15returned";
$P2_lim=30.0;
$P5_lim=0.25;
#$P2_lim=0.05; #test
#$P5_lim=0.01; #test
...
$p2y_lockfile="/tmp/mta/G15pchan_p2y_viol.out";
$p5y_lockfile="/tmp/mta/G15pchan_p5y_viol.out";
$p2r_lockfile="/tmp/mta/G15pchan_p2r_viol.out";
$p5r_lockfile="/tmp/mta/G15pchan_p5r_viol.out";
  ...
  if ($time_diff < 3000 && $p2 > $P2_lim) {
    ...
    if (!-s $p2y_lockfile && !-s $p2r_lockfile) { # if an alert hasn't been sent, send one
      if ($p2_wait == 5) {
        ...
        print OUT "A Radiation violation of GOES 15 P2 (4-9 MeV) has occurred indicating a possible EPHIN P4GM value of 1/3 the threshold.\n";
  ...
  if ($time_diff < 3000 && $p5 > $P5_lim) {
    ...
    if (!-s $p5y_lockfile && !-s $p5r_lockfile) { # if an alert hasn't been sent, send one
      if ($p5_wait == 5) {
        ...
        print OUT "A Radiation violation of GOES 15 P5 (40-80 MeV) has occurred indicating a possible EPHIN P41GM value of 1/3 the threshold.\n";
        printf OUT " Value: %6.2f p/cm2-s-sr-MeV\n",$p5;
        printf OUT " Limit: %6.2f\n", $P5_lim;
        close OUT;
